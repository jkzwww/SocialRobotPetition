...
function recordResp(nameStr,isElement) {
    ...
    if(isElement == 'true'){
        ...
        if(nameStr == 'token')
        {
            var tokenStr = document.getElementById('numToken').innerHTML;
            
            if (tokenStr != "")
            {
                var token_record = "token_" + tokenStr;
                recordData(token_record);
            }
            else
            {
                recordData("signed");
...
// detect idle functionalities
var exitID = 0;

// start timer when new page change 
function waitRestart(exitBoxID)
{
    exitID = exitBoxID;
    myTimeout = setTimeout(autoRestart, 30000);
}

// if idle > 10s, restart page
function autoRestart()
{    
    endButtonArr = document.getElementsByClassName("autoEndBtn");

    if(endButtonArr.length > exitID)
    {
        endButtonArr[exitID].click();
        //clearTimeout(myTimeout);
        // document.getElementById("numToken").innerHTML = "Time up";
    }
    // else
    // {
    //     document.getElementById("numToken").innerHTML = "No button found";

    // }

    console.log(exitID + " page box idle, application restarted.\n")

}

// if interaction detected, stop timer
